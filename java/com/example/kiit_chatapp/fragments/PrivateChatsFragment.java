...
public class PrivateChatsFragment extends Fragment {
    ...
    private Map<String, Long> chatTimestamps = new HashMap<>();
    ...
    // Updated chat loader with sorting
        ...
        chatListRef.child(currentUserId).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot chatSnap : snapshot.getChildren()) {
                    ...
                    long timestamp = 0;
                    if (chatSnap.child("timestamp").exists()) {
                        Object val = chatSnap.child("timestamp").getValue();
                        if (val instanceof Long) timestamp = (Long) val;
                        else if (val instanceof Integer) timestamp = ((Integer)val).longValue();
                    }
                    chatTimestamps.put(otherUid, timestamp);
                ...
                for (String otherUid : uidsToFetch) {
                    usersRef.child(otherUid).addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot userSnap) {
                            ...
                            fetchedCount[0]++;
                            if (fetchedCount[0] == uidsToFetch.size()) {
                                tempUsers.sort((u1, u2) -> {
                                    Long t1 = chatTimestamps.get(u1.getUid());
                                    Long t2 = chatTimestamps.get(u2.getUid());
                                    if (t1 == null) t1 = 0L;
                                    if (t2 == null) t2 = 0L;
                                    return Long.compare(t2, t1); // Descending
                                });
                                chatUsers.clear();
                                chatUsers.addAll(tempUsers);
                                chatListAdapter.notifyDataSetChanged();
                            }
                        }
