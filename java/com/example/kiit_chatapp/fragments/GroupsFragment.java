...
public class GroupsFragment extends Fragment {
        ...
        groupsRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot groupsSnapshot) {
                ...
                for (DataSnapshot groupSnap : groupsSnapshot.getChildren()) {
                    ...
                    boolean isPrivate = group.getInterests().contains("Private");
                    List<String> members = new ArrayList<>();
                    if (groupSnap.child("members").exists()) {
                        for (DataSnapshot memSnap : groupSnap.child("members").getChildren()) {
                            String mem = memSnap.getValue(String.class);
                            if (mem != null) members.add(mem);
                        }
                    }

                    // Only show private group if current user is a member
                    if (isPrivate) {
                        if (members.contains(currentUserId) || members.contains(currentUserEmail)) {
        ...
        for (int idx = 0; idx < allGroupSnapshots.size(); idx++) {
            ...
            groupsRef.child(groupId).child("messages").addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot messagesSnapshot) {
                    ...
                    taggedCounts.set(pos, tagCount);
                    groupsAdapter.notifyItemChanged(pos);
                }
                @Override
                public void onCancelled(@NonNull DatabaseError error) {
                    taggedCounts.set(pos, 0);
                    groupsAdapter.notifyItemChanged(pos);
                }
            });
        }
    ...
    public static class GroupsAdapter extends RecyclerView.Adapter<GroupsAdapter.GroupViewHolder> {
        ...
        public void onBindViewHolder(@NonNull GroupViewHolder holder, int position) {
            ...
            boolean isPrivate = group.getInterests() != null && group.getInterests().contains("Private");

            // --- Private group: always "View Group", never "Join" ---
            if (isPrivate) {
                holder.joinButton.setText("View Group");
                holder.joinButton.setEnabled(true);
                holder.joinButton.setOnClickListener(v -> {
                    if (actionClickListener != null) {
                        actionClickListener.onActionClick(group, position, true); // Always treat as "joined" for private
                    }
                });
            } else {
